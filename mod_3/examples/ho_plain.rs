...
impl HO {
    fn v(&self, x: f64) -> f64 {
        self.m * self.omega.powi(2) * x.powi(2) / 2.0
        //x.powi(10)
    }
...
impl LatticeStructure for HO {
    ...
    fn delta_euclid_action(lattice: &mod_3::Lattice<Self>, time_index: usize, trial: &Self::Guess) -> f64 {
        ...
        let se_old = Self::local_euclid_action(
            x(i),
            x(i + 1),
            lattice
        ) + Self::local_euclid_action(
            x(i - 1),
            x(i),
            lattice
        );

        let se_new = Self::local_euclid_action(
            x_trial,
            x(i + 1),
            lattice
        ) + Self::local_euclid_action(
...
fn main() {
    ...
    for i in 0..N_REP {
        ...
        if i % MEASURE_EVERY == 0 {
            ...
            writeln!(file, "{}", x2 + p2_corrected).unwrap();
